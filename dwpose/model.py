...
from .util import draw_bodypose, draw_facepose, draw_handpose, hwc3, safe_resize
...
class DWPoseDetector:
    ...
    def from_pretrained(
        cls,
        repo_id="benjamin-paine/taproot-common",
        detector_file_name="pose-detection-dwpose-detection.safetensors",
        estimator_file_name="pose-detection-dwpose-estimation.safetensors",
        device="cpu",
        dtype=None,
        do_eval=True,
        do_compile=False,
        ...
        repo_path = snapshot_download(
            repo_id=repo_id,
            allow_patterns=[detector_file_name, estimator_file_name],
        )
        det_ckpt = os.path.join(repo_path, detector_file_name)
        pose_ckpt = os.path.join(repo_path, estimator_file_name)
        ...
        with torch.no_grad():
            ...
            if isolated:
                ...
                for key in ["bodies", "hands", "faces"]:
                    if (key == "bodies" and pose[key]["subset"].any()) or (
                        key != "bodies" and pose[key].any()
                    ):
                        these_maps = []
                        this_pose = {
                            "bodies": {
